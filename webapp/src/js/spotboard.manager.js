define([
   ...
   * @return $.Deferred
   */
  Spotboard.Manager.feedSingleRun = function (
    ...
    var fed = runfeeder.feed(1, function (run, remainCount) {
      Spotboard.Manager.displayedContestTime =
        Spotboard.runfeeder.getContestTime(); // TODO 중복
      ...
      var fnUpdateTeamRank = function () {
        $("#team-list > .team").each(function () {
          ...
          var newRank = contest
            .getTeamStatus($(this).data("team-id"))
            .getRank();
          if (oldRank != newRank) {
        ...
        if (!Spotboard.config["award_mode"]) {
          // TODO 조건이 좀 구림..
  ...
  Spotboard.Manager.isTeamExcluded = function (team) {
    Spotboard.Manager._isExcluded =
      Spotboard.Manager._isExcluded ||
      (function () {
        var excludeTeams = Spotboard.config["exclude_teams"] || {};
        if (typeof excludeTeams === "function") return excludeTeams;
  ...
  Spotboard.Manager.initButtonEventHandlers = function () {
    $("#feed-one-icon").click(function () {
      Spotboard.Manager.proceedOne();
    });
    $("#feed-all-icon").click(function () {
      Spotboard.Manager.feedAll();
    });
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          fn.call(runfeeder, data.data, function filter(r) {
            ...
            if (
              contest
                .getTeamStatus(r.getTeam())
                .getProblemStatus(r.getProblem())
                .isAccepted()
            )
              return false;
            // 제외되는 팀의 런은 피드하지 않음
            if (Spotboard.Manager.isTeamExcluded(r.getTeam())) return false;
            return true;
          });
  ...
  Spotboard.Manager.initSearchEventHandlers = function () {
    ...
    if (config["search_query"]) {
      $("#search-input").val(config["search_query"]).trigger("input");
    }
  };
  ...
  Spotboard.Manager.initUpdateTimer = function () {
    ...
    function renew_timer() {
      ...
      var refreshInterval =
        parseInt(Spotboard.config["auto_refresh_interval"]) || 10000;
  ...
  Spotboard.Manager.feedInitialRuns = function () {
    var runfeeder = new RunFeeder(
      Spotboard.contest,
      new FIFORunFeedingStrategy()
    );
